                    ...
                    for idx in (7, 9, 11):
                        if participant[idx]:
                            ...
                            if parsed_date == final_exam.date_time.date():
                                student.add_grade(model, StudentGrade(model, student, final_exam, Grade[participant[idx + 1]]))
                                break
                    else:
                        student.add_grade(model, StudentGrade(model, student, final_exam, Grade.FX))
